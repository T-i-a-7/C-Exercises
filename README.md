![Banner](Banner.png)

<h1 align="center">
   C Exercises - Archive
</h1>

<p align="center">
   A collection of exercises in the C programming language.
</p>

<p align="center">
   <img src="https://img.shields.io/badge/Exercises-70%2B-87D3F8" alt="Number of Exercises">
   <img src="https://img.shields.io/badge/Difficulty-Medium-FFA500" alt="Difficulty Medium">
   <img src="https://img.shields.io/badge/Language-C-00599C" alt="Language C">
   <img src="https://img.shields.io/badge/License-MIT-green" alt="License MIT">
</p>

</br>


## 📖 About

**C Exercises - Archive** is a personal collection of programming exercises in the C language that I have completed over the years during my Programming classes.
This repository represents an effort to organize these exercises in one place.

By making them public, I hope they can serve as a resource for others who are learning or practicing C programming.
Whether you are a student, a self-learner, or someone looking to brush up on C, this archive aims to provide useful exercises for a wide range of programming topics.

## 📂 Repository Structure

Exercises are mainly organized by year and date:

```text
C-Exercises/
├── 2023/
├── 2024/
├── 2025/
└── Extras/
````

> [!TIP]
> If you are new to C programming, it is recommended to start with the older exercises.

Where available, the original task, prompt, or problem statement is included as a comment in the corresponding `.c` file. The `Extras` directory contains additional exercises that do not belong to the main chronological structure.

## 🗃️ Archive Policy

> [!IMPORTANT]
> This repository is a historical archive, not a collection of reference solutions.
> The exercises are intentionally preserved close to how I originally wrote them, so some may contain mistakes, incomplete implementations, inefficient approaches, or code that I would write differently today.

This is intentional: the goal is to preserve the original learning process rather than turn every exercise into an ideal solution.

## 💻 Using the Exercises

You are welcome to browse the repository, study the exercises, or use them as inspiration for your own practice.

To clone the archive:

```bash
git clone https://github.com/T-i-a-7/C-Exercises.git
```

If you want to modify, correct, complete, or experiment with the exercises, creating your own fork is recommended.

> [!NOTE]
> The code in this repository should not be treated as authoritative or production-ready C code. Some exercises reflect early stages of my learning and may contain errors or incomplete solutions.

## 🤝 Contributions and Feedback

This repository is primarily a personal archive, so changes that rewrite, correct, complete, or replace historical exercise solutions are generally not intended to be merged into the archive.

Feedback and contributions related to the repository itself — such as documentation, organization, broken references, or metadata — are still welcome.

Alternative or improved solutions are best explored in a fork, where the original archived versions can remain unchanged.

## 📄 License

This repository is licensed under the [MIT License](LICENSE).
