Si deve fornire un programma in C, con vettori di strutture, ad una società che gestisce le farmacie di una città. Tale programma prevede di poter inserire le seguenti informazioni:
...
Ogni farmaco è caratterizzato da una fascia: la fascia C indica i farmaci a carico del sistema sanitario nazionale (quindi gratuiti per gli assistiti), la fascia H indica i prodotti ospedalieri (quindi acquistabili solo dagli ospedali) ed infine la fascia A che indica i prodotti da banco completamente a carico degli assistiti. In ogni caso ogni farmaco ha un costo inserito.
...
La prima richiesta prevede l'inserimento di un nuovo farmaco e di tutte le informazioni che lo riguardano, chiedendo all'utente solo quelle necessarie. Il nuovo farmaco si deve inserire in fondo al vettore.
